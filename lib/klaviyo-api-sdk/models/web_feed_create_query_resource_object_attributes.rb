  ...
  class WebFeedCreateQueryResourceObjectAttributes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KlaviyoAPI::WebFeedCreateQueryResourceObjectAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KlaviyoAPI::WebFeedCreateQueryResourceObjectAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @request_method.nil?
        invalid_properties.push('invalid value for "request_method", request_method cannot be nil.')
      end
    ...
    def valid?
      ...
      content_type_validator = EnumAttributeValidator.new('String', ["json", "xml"])
    ...
    def hash
      [name, url, request_method, content_type].hash
    end
