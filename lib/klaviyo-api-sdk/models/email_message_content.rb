  ...
  class EmailMessageContent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `KlaviyoAPI::EmailMessageContent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `KlaviyoAPI::EmailMessageContent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @preview_text.nil?
        invalid_properties.push('invalid value for "preview_text", preview_text cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @preview_text.nil?
    ...
    def hash
      [subject, preview_text, from_email, from_name].hash
    end
